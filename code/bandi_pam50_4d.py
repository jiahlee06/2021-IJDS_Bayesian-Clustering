...
if __name__ == "__main__":
    ...
    # edit here
    in_csv = "processed/pam50-4d.csv"
    out_csv = "model/bandi_pam50-4d.csv"
    col_names = ['component1', 'component2', 'component3', 'component4']
    num_components = 5
    d = 4
    ...
    mip_means, mip_covariances, mip_weights = model.optimize(verbose=False, timelimit=3600)
    ...
    bayes_p = [np.linalg.det(mip_covariances[k])**(-0.5) * np.exp(-0.5*((features[i] - mip_means[k]) @ np.linalg.pinv(mip_covariances[k]) @ (features[i] - mip_means[k]).T)) for i, k in product(range(features.shape[0]), range(num_components))]
