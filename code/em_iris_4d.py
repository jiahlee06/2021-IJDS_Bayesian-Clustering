...
def fit_gmm(mod: dict, y):
    ...
    def update_m():
        return np.dot(mod['tau'].T, y) / np.repeat(np.sum(mod['tau'],
                                                          axis=0), 4).reshape((K, 4))
    def update_cov(mu):
        temp_cov = np.zeros((K,4,4))
        for k in range(K):
...
def init_kmeans(kmeanscsv):
    ...
    mod['m'] = np.array(mod['m']).reshape((3, 4))
    mod['p'] = np.array(mod['p'])
    mod['cov'] = np.array(mod['cov']).reshape((3, 4, 4))

    return mod


def gmm_iris_4d(incsv, kmeanscsv, outcsv):
...
gmm_iris_4d("processed/iris-4d.csv","model/kmeans_iris-4d.csv",
         "model/em_iris-4d.csv")
